...
fit_longitudinal_cdm_mixed=function(Ys,Xs,Qs,M,Z,
  ...
  if(gen_init){
    ...
    group_params=matrix(rnorm(nX*nZ,sd=1),nZ,nX)
    indv_params=map(1:nG,~spam::rmvnorm(1,(Z%*%gamma)[.,],Sigma=diag(nX)))%>%
      {do.call(rbind,.)}
    Sigma=rWishart(1,diag(nX),df=3)[,,1]
    ml_params=list(group_params,indv_params,Sigma)
    ml_priors=list(
      V_prior=diag(nX),
      v_prior=1,
    ...
    # ml_params=map(Xs,~map(.,function(x) rnorm(dim(x)[2])))
  }else{
    beta_mat=initparams$beta_mat
    ml_params=initparams$ml_params
  }



  # ml_params=ml_params_true
  ...
  gamma_vec=unlist(ml_params)

  trans_probs=gamma_to_transprobs(ml_params,Xs)          # logistic(psi)
  ...
  #MCMC setup
  {
    ...
    samples=matrix(NA,M,length(sampnames))%>%
      as.data.frame()%>%
      set_names(sampnames)
    samples[1,]=c(beta_vec,unlist(ml_params),alpha_vec)
  ...
  if (fixed_beta)
  {
    for(m in 2:M){
      ...
      #sample alpha
      trans_probs=gamma_to_transprobs(ml_params,Xs)
      ...
      #sample gamma
      # ml_params=sample_gamma(ml_params,trans_mat,Xs,priorsd_gamma)

      ml_params=list()
      for(k in 1:Nskill){
          multi_level_samples=
            sample_ml_pg(trans_mat[k,],X[[k]],Z,
                         indv_params,group_params,Sigma,group_assignments,ml_priors)
          ml_params[[k]]=multi_level_samples
          # indv_params=multi_level_samples$beta
          # group_params=multi_level_samples$gamma
          # Sigma=multi_level_samples$Sigma
      }

      samples[m,]=c(beta_vec,unlist(ml_params),alpha_mat)
  ...
  # gamma_post=sample_gamma(ml_params,trans_mat,Xs,priorsd_gamma,retmean=T)
